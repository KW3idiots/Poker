...
int main() {
	...
	set_table();
	for (int i = 0; i < 6; i++) {
		set_card(SPADE, 11, i, 0);
		set_card(SPADE, 9, i, 1);
	}

	set_card(DIAMOND, 9, 5, 2);
	set_card(SPADE, 9, 5, 3);
	set_card(SPADE, 9, 5, 4);

	char A[6][22] = {"aa", "bbb", "cccccc", "dddd", "eeeee"};

	for (int i = 0; i < 5; i++) {
		set_player_name(i, A[i]);
	}
	
	set_player_capital(4, 12345690);
	set_player_capital(0, 123457890);
	set_player_capital(1, 1267890);
	set_player_capital(2, 34567890);
	set_player_capital(3, 12);
	set_player_capital(5, 123245690);
	set_player_hand(0, STRAIGHT);
